...
class TestMouseAxisConstraint(unittest.TestCase):
    ...
    def test_mask_four_keeps_only_z(self):
        ...
        start = [1.0, 2.0, 3.0]
        candidate = [9.0, 8.0, 7.0]
        self.assertEqual(handler._apply_axis_constraint(start, candidate), [1.0, 2.0, 7.0])

    def test_mask_three_keeps_xy(self):
        handler = self._make_handler(3)
        start = [1.0, 2.0, 3.0]
        candidate = [9.0, 8.0, 7.0]
        self.assertEqual(handler._apply_axis_constraint(start, candidate), [9.0, 8.0, 3.0])
